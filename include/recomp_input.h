...
namespace recomp {
    ...
    #define DEFINE_N64_BUTTON_INPUTS() \
        DEFINE_INPUT(A, 0x8000, "动作") \
        DEFINE_INPUT(B, 0x4000, "攻击/取消") \
        DEFINE_INPUT(Z, 0x2000, "锁定") \
        DEFINE_INPUT(START, 0x1000, "开始") \
        DEFINE_INPUT(L, 0x0020, "切换地图") \
        DEFINE_INPUT(R, 0x0010, "盾牌") \
        DEFINE_INPUT(C_UP, 0x0008, "观察/妖精") \
        DEFINE_INPUT(C_LEFT, 0x0002, "道具 1") \
        DEFINE_INPUT(C_DOWN, 0x0004, "道具 2") \
        DEFINE_INPUT(C_RIGHT, 0x0001, "道具 3") \
        DEFINE_INPUT(DPAD_UP, 0x0800, "特殊道具 1") \
        DEFINE_INPUT(DPAD_RIGHT, 0x0100, "特殊道具 2") \
        DEFINE_INPUT(DPAD_DOWN, 0x0400, "特殊道具 3") \
        DEFINE_INPUT(DPAD_LEFT, 0x0200, "特殊道具 4")

    #define DEFINE_N64_AXIS_INPUTS() \
        DEFINE_INPUT(Y_AXIS_POS, 0, "上") \
        DEFINE_INPUT(Y_AXIS_NEG, 0, "下") \
        DEFINE_INPUT(X_AXIS_NEG, 0, "左") \
        DEFINE_INPUT(X_AXIS_POS, 0, "右") \

    #define DEFINE_RECOMP_UI_INPUTS() \
        DEFINE_INPUT(TOGGLE_MENU, 0, "打开/关闭菜单") \
        DEFINE_INPUT(ACCEPT_MENU, 0, "确认（菜单）") \
        DEFINE_INPUT(APPLY_MENU, 0, "应用（菜单）")
