...
ModDetailsPanel::ModDetailsPanel(Element *parent) : Element(parent) {
    ...
    buttons_container->set_background_color(Color{ 0, 0, 0, 89 });
    {
        ...
        enable_container->set_gap(16.0f);
        {
            ...
            enable_label = context.create_element<Label>(enable_container, "当前启用的模组依赖此模组", LabelStyle::Annotation);
        }

        configure_button = context.create_element<Button>(buttons_container, "设置", recompui::ButtonStyle::Secondary);
