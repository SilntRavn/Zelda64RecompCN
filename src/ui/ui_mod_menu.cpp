...
ModMenu::ModMenu(Element *parent) : Element(parent) {
    ...
    
    {
        ...
        body_empty_container->set_display(Display::None);
        {
            ...
            context.create_element<Label>(body_empty_container, "尚未安装模组", LabelStyle::Large);
            context.create_element<Element>(body_empty_container);
        ...
        {
            Button* configure_button = mod_details_panel->get_configure_button();
            install_mods_button = context.create_element<Button>(footer_container, "安装模组", recompui::ButtonStyle::Primary);
            ...
            refresh_button = context.create_element<Button>(footer_container, "刷新", recompui::ButtonStyle::Primary);
            refresh_button->add_pressed_callback([this](){ refresh_mods(true); });
            refresh_button->set_nav_manual(NavDirection::Up, mod_tab_id);

            mods_folder_button = context.create_element<Button>(footer_container, "打开模组文件夹", recompui::ButtonStyle::Primary);
