...
extern "C" void recomp_get_third_person_mouse_deltas(uint8_t* rdram, recomp_context* ctx) {
    float* x_out = _arg<0, float*>(rdram, ctx);
    float* y_out = _arg<1, float*>(rdram, ctx);

    recomp::get_third_person_mouse_deltas(x_out, y_out);
}
...
extern "C" void recomp_get_game_speed_multiplier(uint8_t* rdram, recomp_context* ctx) {
    _return(ctx, ultramodern::get_speed_multiplier());
}
